...
For the lab a computational intensive kernel has to be extracted from the \mcode{x264} software application: a free software library for encoding video stream into H.236/MPEG-4 AVC format. It is able to use Periodic Intra Refresh instead of keyframes as used by h.264, refreshing the image of the video by moving a column of intra blocks from one side of the screen to the other. This hides the refreshing effect from the user while the frame loads. The \mcode{x264} application is executed on a FPGA running a MicroBlaze host processor. By running the particular kernel on the $\rho$-VEX co-processor, the execution time can be decreased by making use of parallel processing. 
...
\label{sec:intensive}
A few input files are provided in the first lab to show the compile and run commands of the x264 application. These \mcode{.y4m} input files can be decoded to create a short \mcode{.mkv} movie using this command in the terminal: \\
